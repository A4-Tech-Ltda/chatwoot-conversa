...
class DataImportJob < ApplicationJob
  ...
  def import_contacts(contacts)
    ...
    apply_pending_labels(contacts)
  end

  def apply_pending_labels(contacts)
    pending = @contact_manager.pending_labels
    return if pending.empty?

    contacts.each do |contact|
      label = pending[contact.object_id]
      next unless label && contact.persisted?

      contact.update_labels([label])
    end
  end
